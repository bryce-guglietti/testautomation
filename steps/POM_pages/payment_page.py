...
class PaymentPage:
    ...
    def __init__(self, driver):
        ...
        self.confirm_button = self.driver.find_element(by=By.XPATH, value ="//button[@id='submit']")
    ...
    def fill_payment_info(self, name, number, cvc, expiry_month, expiry_year):
        ...
        name_field = self.driver.find_element(by=By.XPATH, value = "//form[@id='payment-form']/*/*/input[@name='name_on_card']")
        name_field.send_keys(name)

        # Locate the number field with XPath and fill it with the parameter
        number_field = self.driver.find_element(by=By.XPATH, value = "//form[@id='payment-form']/*/*/input[@name='card_number']")
        number_field.send_keys(number)

        # Locate the cvc field with XPath and fill it with the parameter
        cvc_field = self.driver.find_element(by=By.XPATH, value = "//form[@id='payment-form']/*/*/input[@name='cvc']")
        cvc_field.send_keys(cvc)

        # Locate the expiry month field with XPath and fill it with the parameter
        exp_field = self.driver.find_element(by=By.XPATH, value = "//form[@id='payment-form']/*/*/input[@name='expiry_month']")
        exp_field.send_keys(expiry_month)

        # Locate the expiry year field with XPath and fill it with the parameter
        exp_year_field = self.driver.find_element(by=By.XPATH, value = "//form[@id='payment-form']/*/*/input[@name='expiry_year']")
